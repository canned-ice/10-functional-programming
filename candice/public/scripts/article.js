...
(function(module) {
  ...
  module.Article.numWordsAll = () => {
    return app.Article.all.map( article => article.body.split(' ').length).reduce( (acc, curr) => acc + curr);
  };

  module.Article.allAuthors = () => {
    return app.Article.all.map( value => value.author ).reduce( (acc, curr) => { if (!acc.includes(curr)) {acc.push(curr)} return acc}, []);
  };

  module.Article.numWordsByAuthor = () => {
    return app.Article.allAuthors().map(auth => {
      return {
        author: auth,
        words: app.Article.all.filter( value => value.author === auth ).map( value => value.body.split(' ').length).reduce( (acc, curr) => acc + curr)
      }
    });
  };
  ...
  module.Article.prototype.insertRecord = function(callback) {
    ...
    // Due to the usage of this in the function, as it would not allow the use of that term in an arrow function (it would be undefined)
